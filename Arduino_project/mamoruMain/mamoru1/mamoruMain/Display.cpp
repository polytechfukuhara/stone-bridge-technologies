...
*/
void Display::setPosAndSize(int posX, int posY, int showSizeNum) {
  _x = posX;
  _y = posY;
...
/**
   バッテリー残量の表示を更新する関数
*/
void Display::batteryUpdate(int percent = -1) {
  if (!_showFlg) {
    return;
  }
  // 電池図形内部背景塗りつぶし
  ...
  i_percent = percent;
  if (i_percent == -1) {
    i_percent = battery.calcBatteryPercent();
  }
}
  /**
     削除時の背景色を設定する関数
  */
  void Display::setDeleteBgColor(uint32_t color) {
    _bg_color  = color;
  }
  /**
     電池図形と文字の色を設定する関数
  ...
  */
  ...
  /**
     バッテリー残量を表示する関数
  */
  void Display::showBatteryPercent(int i_percent) {
    // バッテリー数値を表示
  ...
  */
  ...
  /**
     main
  */
  void Display::batteryShow_Setup() {
...
/**
   振動検知画面表示するを関数
*/
void Display::showEmergencyMove() {
...
*/
//Bluetooth接続画面
void Display::BluetoothSearching() {
  // LCD表示設定
  ...
  bth.SerialDataRead(); // 電源ON時のシリアルデータが無くなるまで待つ
